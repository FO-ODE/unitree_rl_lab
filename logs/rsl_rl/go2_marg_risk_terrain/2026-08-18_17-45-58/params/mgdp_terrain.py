...
def _mgdp_boxes_terrain(
    difficulty: float,
    cfg: "MGDPBoxesTerrainCfg",
) -> tuple[list[trimesh.Trimesh], np.ndarray]:
    """Tile Isaac Lab's square boxes terrain over the rectangular MGDP footprint."""
    tile_size = min(cfg.size)
    tiles_x = int(round(cfg.size[0] / tile_size))
    tiles_y = int(round(cfg.size[1] / tile_size))
    if not np.isclose(tiles_x * tile_size, cfg.size[0]) or not np.isclose(tiles_y * tile_size, cfg.size[1]):
        raise ValueError(f"MGDP boxes terrain size {cfg.size} must be divisible into square tiles.")

    tile_cfg = cfg.copy()
    tile_cfg.size = (tile_size, tile_size)
    meshes: list[trimesh.Trimesh] = []
    spawn_origin: np.ndarray | None = None
    for tile_x in range(tiles_x):
        for tile_y in range(tiles_y):
            tile_meshes, tile_origin = mesh_terrains.random_grid_terrain(difficulty, tile_cfg)
            translation = np.array([tile_x * tile_size, tile_y * tile_size, 0.0])
            for mesh in tile_meshes:
                mesh.apply_translation(translation)
            meshes.extend(tile_meshes)
            if spawn_origin is None:
                spawn_origin = tile_origin + translation

    if spawn_origin is None:
        raise RuntimeError("MGDP boxes terrain did not generate any square tiles.")
    return meshes, spawn_origin


@configclass
class MGDPBoxesTerrainCfg(terrain_gen.MeshRandomGridTerrainCfg):
    function = _mgdp_boxes_terrain
...
MGDP_GAP_PARKOUR_TERRAIN_GENERATOR_CFG = TerrainGeneratorCfg(
    ...
    sub_terrains={
        **{
        ...
        "boxes": MGDPBoxesTerrainCfg(
            proportion=MGDP_GAP_PARKOUR_WEIGHTS["boxes"],
            grid_width=0.45,
