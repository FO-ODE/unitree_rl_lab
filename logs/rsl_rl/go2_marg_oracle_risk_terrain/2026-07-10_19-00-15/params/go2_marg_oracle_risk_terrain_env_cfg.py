...
def _transform_proprio_history_left_right(obs: torch.Tensor) -> torch.Tensor:
    ...
    # ObservationManager flattens the history of each term first, then
    # concatenates the terms. Reconstruct the six 45D frames before applying
    # the per-frame left-right transform, and finally restore term-major order.
